...
Ollama-Instanz (`ollama serve`), zieht bei Bedarf das Modell
`qwen3-embedding:0.6b` und nutzt diese für die Anfrage. Du musst den Dienst
nicht separat starten; falls du lieber einen anderen Endpoint oder ein anderes
Modell verwenden möchtest, kannst du das über die Parameter `endpoint`/`model`
in `marktview.llm.infer_gender_for_listing` tun.
